...
class TicTacToe:
    ...
    def init_game_grid(self):
            ...
            for j in range(self.squares_num):
                self.clickables.append(
                    Clickable(
                        ...
                        y=int(
                            self.y_offset + j * self.square_size + self.grid_thickness
                        ),
                        w=int(self.square_size - self.grid_thickness),
                        h=int(self.square_size - self.grid_thickness),
        ...
        self.info_player1 = nepygui.TextFrame(
            fill=Color.Gray,
        ...
        self.info_player2 = nepygui.TextFrame(
            fill=Color.Gray,
            fontcolor=Color.Red,
            fontsize=14,
            bold=True,
        ...
        self.info_player1_points = nepygui.TextFrame(
            fill=Color.Gray,
            fontcolor=Color.Blue,
            fontsize=14,
            bold=False,
        ...
        self.info_player2_points = nepygui.TextFrame(
            fill=Color.Gray,
            fontcolor=Color.Red,
            fontsize=14,
            bold=False,
        ...
        # whose turn
        self.whose_turn = nepygui.TextFrame(
            fill=Color.Gray,
        ...
        self.new_game_button = nepygui.Button(
            ...
            gradient=False,
            fill=Color.DarkGray,
            func=lambda btn: self.make_new_game,
        )
    ...
    def check_result(self):
        if (
            self.check_vertical(self.board, "X")
            or self.check_horizontal(self.board, "X")
            or self.check_diagonally(self.board, "X")
        ):
            ...
            self.check_vertical(self.board, "O")
            or self.check_horizontal(self.board, "O")
            or self.check_diagonally(self.board, "O")
        ):
            if self.game_running:
                self.resize_grid()
    ...
    def check_horizontal(self, whole, mark):
        return (
            all(elem[0] == mark for elem in whole)
            or all(elem[1] == mark for elem in whole)
            or all(elem[2] == mark for elem in whole)
        )
...
class Clickable(nepygui.Button):
    ...
    def draw_X(self, screen, t):
        ...
        pygame.draw.line(
            screen,
            Color.Blue,
            (self.x + space, self.y + space),
            (self.x + self.w - space, self.y + self.h - space),
            t,
        )
        pygame.draw.line(
            screen,
            Color.Blue,
            (self.x + space, self.y + self.h - space),
            (self.x + self.w - space, self.y + space),
            t,
        )
